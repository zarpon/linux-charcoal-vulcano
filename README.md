...
| [zram-ir](https://github.com/firelzrd/zram-ir) | Adds immediate zram recompression control through `vm.zram_recomp_immediate`. The Charcoal kernel port defaults that control to `2`, so the write path tries all three configured priorities: LZ4 primary, LZO-RLE priority `1`, then ZSTD priority `2`. The bundled kernel port fixes ZSTD's ZRAM compression level at `1`; userspace `algorithm_params` cannot override it. A packaged `zram-generator` drop-in and `systemd-zram-setup@` `ExecStartPre` configure the three algorithms before `disksize`. The udev helper reasserts the sysctl and provides a safe fallback; it never resets an initialized device or active swap and does not create an additional zram swap device. |
...
primary compressor, LZO-RLE priority-`1` recompressor, and ZSTD level-`1`
priority-`2` recompressor apply on the first boot into Charcoal. Verify them
after that reboot:
...
`[lz4]` marks the selected primary compressor. In `recomp_algorithm`, LZO-RLE
is shown in the priority-`1` row and ZSTD in priority `2`. ZSTD level `1` is
fixed in the Charcoal ZRAM-IR kernel port and cannot be overridden by a
userspace `algorithm_params` setting.
