...
UA = "linux-charcoal-TD-dynamic-resolver/2"
...
def upstream_candidates(component: dict[str, Any], kernel_version: str, token: str | None) -> list[Candidate]:
    ...
    for item in tree.get("tree", []):
        ...
        if exclude and exclude.search(path):
            continue
        compatibility = 3 if kernel_version in path else 2 if series in path else 1 if "latest" in path.lower() else 0
        if compatibility:
...
def resolve_component(component: dict[str, Any], kernel_version: str, token: str | None, root: Path) -> dict[str, Any]:
    ...
    if local_port:
        ...
        if not path.is_file():
            raise ResolveError(f"no compatible upstream patch for {component['name']} and local port missing: {local_port}")
        data = path.read_bytes()
        if not data.startswith((b"From ", b"diff --git", b"--- ")):
            raise ResolveError(f"local port does not look like a patch: {local_port}")
        return {
            "repository": "zarpon/linux-charcoal-TD", "path": local_port,
            "commit": "repository-local", "url": None,
            "origin": "local-port", "content_bytes": data
        }
...
def main() -> int:
    ...
    if args.fixture:
        ...
        selected = {component["name"]: resolve_component(component, kernel_version, token, root) for component in manifest["components"]}
    kernel_version = kernel_tag.split("-valve", 1)[0]
    lock: dict[str, Any] = {"schema": 2, "kernel": {"tag": kernel_tag, "version": kernel_version, "commit": kernel_sha}, "components": {}}
    ...
    for component in manifest["components"]:
        ...
        lock["components"][name] = {**clean, "target": target, "sha256": hashlib.sha256(data).hexdigest(), "size": len(data)}
