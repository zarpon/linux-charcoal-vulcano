...
require_line "$root/config-charcoal" "CONFIG_CRYPTO_LZ4KDR=y"
...
grep -Fq 'config CRYPTO_LZ4KDR' "$root/0002-linux6.16.12-lz4kdr-zswap-1.0.patch" || fail "LZ4KDR zswap Kconfig is missing"
grep -Fq 'zswap.compressor=lz4kdr' "$root/0002-linux6.16.12-lz4kdr-zswap-1.0.patch" || fail "LZ4KDR zswap boot selector is missing"
if grep -Fq 'drivers/block/zram/' "$root/0002-linux6.16.12-lz4kdr-zswap-1.0.patch"; then
  fail "zswap adapter patch unexpectedly changes the zram backend"
fi
